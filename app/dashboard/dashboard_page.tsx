...
export default function Dashboard() {
  ...
  async function generateNow() {
    setGenerating(true);
    try {
      // Always save whatever's currently typed first — otherwise this would
      // silently generate against the last *saved* topics instead of what's
      ...
      <div className="glass-card" style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 18 }}>
          ...
          <button
            ...
            disabled={generating}
          >
            {generating ? 'Researching + writing…' : 'Save topics & generate now'}
          </button>
        </div>
        <p style={{ fontSize: 11.5, color: 'var(--text-low)', marginTop: 10 }}>
          This saves your current topics automatically before generating, so whatever's in the Topics field above is what gets used.
        </p>
      </div>
